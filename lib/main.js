...
function buildProject(hub, project, callback) {
    ...
    pipeEvents(build, ["stdout", "stderr", "exec", "finished", "success", "failure"]);
    build.on("finished", function(code) {
        callback();
    });

    function pipeEvents(from, names) {
        names.forEach(function(name) {
            from.on(name, function() {
                var args = [eventPrefix + "." + name].concat(Array.prototype.slice.call(arguments, 0));
