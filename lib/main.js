...
function setUpProject(hub, project) {
    hub.on("time.minute", function() {
        async.map(project.repositories, fetchRepository, function(err, repos) {
            if (err) {
                // TODO: do something
    ...
    hub.on("projects." + project.name + ".builds.*.start", buildProject.bind(null, project));
    
    function fetchRepository(repository, callback) {
