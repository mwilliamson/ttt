...
function setUpProject(hub, project) {
    project.lock = locks.createLock();
    hub.on("time.minute", project.lock.wrap(updateSources));
    ...
    hub.on(startBuildName, function() {
        hub.emit("projects." + project.name + ".builds." + nextBuildNumber + ".start");
        nextBuildNumber += 1;
    });
    
    hub.on("projects." + project.name + ".builds.*.start", function() {
        project.lock.wrap(buildProject.bind(null, project))(function(){});
    });
