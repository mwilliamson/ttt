...
function setUpProject(hub, project) {
    ...
    hub.on("projects." + project.name + ".force-build", function() {
        ...
        queue.add(startBuildName, function(callback) {
            buildProject(hub, project, callback)
        });
    });
...
function buildProject(hub, project, callback) {
    ...
    var buildSteps = project.build.map(function(command) {
        return function() {
            hub.emit(eventPrefix + ".exec", command);
            return child_process.exec(command, {cwd: project.path});
        };
    });
    ...
    process.on("stdout", function(chunk) {
        hub.emit(eventPrefix + ".stdout", chunk);
    });
    process.on("stderr", function(chunk) {
        hub.emit(eventPrefix + ".stderr", chunk);
    });
    process.on("exit", function(code) {
        if (code === 0) {
            hub.emit(eventPrefix + ".success");
        } else {
            hub.emit(eventPrefix + ".failure");
        }
        
        hub.emit(eventPrefix + ".finished");
        callback();
    });
}
